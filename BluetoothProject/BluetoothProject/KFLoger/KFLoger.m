...
- (void)dealloc
{
    free(_curSourceFileName);
    free(_curFuncName);
    free(_logTime);
    
    [_logTextContent release];
...
- (id)init
{
    if (self = [super init])
    {
        ...
        _filePath = [[path objectAtIndex:0] retain];
...
- (void)writeOneLog:(KFLogInfo*)logInfo
{
    ...
    NSString* logFileName = [[self createLogFile] retain];
...
- (char*)getFileName:(const char*)filePath
{
    ...
    int len = (int)strlen(filePath);    //传入路径的总长度
...
- (void)addLog:(NSString*)txtContent sourceName:(const char*)sourceName funcName:(const char*)funcName lineNum:(int)lineNum logType:(NSInteger)logType isNeedSave:(BOOL)isNeedSave
{
    ...
    //控制台打印日志
#ifdef YLL_DEBUG_KFLOG_PRINT_CONSOLE
    ...
    // 这种情况是所有的日志都写文件
#ifdef YLL_DEBUG_KFLOG_WRITE_FILE
    if (isNeedSave)
    {
        // 加个锁，多线程保护
        @synchronized(_logArray)
